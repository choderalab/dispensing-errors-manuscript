...
{\color{red}[JDC: Should we be more explicit that what is observed---product accumulation?---is modeled by the velocity times time, $V_0 t$, assuming that there is an excess of substrate, and that we just report this relative to the uninhibited enzyme---$(V_0 t) / (V_\mathrm{max} t) = V_0 / V_\mathrm{max}$?]}
...
\begin{figure}[tb]
  ...
  While the use of an air gap (white) reduces the magnitude of this dilution effect, it is still a known problem in fixed tip liquid-based automated liquid handling technologies, requiring more complex liquid-handling strategies to eliminate it~\cite{gu_dilution_2007}. 
...
{\color{red}[JDC: This section needs help!  The two most important things are (1) there is a lower limit to observable fluorescence, and (2) some noise should be added.  Do we want to just talk about this but leave out the effects, or do we want to try to incorporate this here and regenerate the figures?]}
...
It is easy to see that simple differences in these values, even when including the compounded error of the dilution series, does not give us much insight into the striking differences seen in Ekins et al.~\cite{ekins_dispensing_2013} between tip-based and direct dispensing.
...
{\color{red}[JDC: This section seems anemic. I think we should either move it entirely to be a part of the Discussion or somehow elaborate on this here.]}
...
\begin{figure}[tb]
  ...
  {\color{red}[JDC: Is red without any bias, and green with the bias for the model that accounts for tip dilution?  Or is red the model including bias from the non-tip-dilution model, and green the model including bias from the tip-dilution model?  Should we show all three?  No bias, bias correction without dilution, bias correction with dilution?]}
...
Here, we have demonstrated how a simple model based on the bootstrap principle, in which nothing more than the manufacturer-provided imprecision and inaccuracy values and a description of the protocol were used to \emph{simulate} virtual replicates of the experiment for a variety of simulated compound affinities, allowed us to estimate the imprecision and inaccuracy of measured IC$_{50}$s, as well as identify the difficulty in creating an accurate dilution series using a fix-tip dispenseras the major contribution to discrepancies in measurements between fixed pipette tip and direct dispensing technologies. 
In addition to providing some estimate for the random error in measured affinities, the computed bias could even be used to \emph{correct} for the bias introduced by this process after the fact. 
...
This assay is just one example of the large class of assays for be done if this model is pursued for other types of experiments relevant to computational modelers.
...
Since the model is quantitative, tinkering with the assay protocol could allow the experimentalist to optimize the protocol to make sure the data is appropriate to the question at hand. 
